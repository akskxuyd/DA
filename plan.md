# SESI 2026-08-19 (#27) — **FASE G DITUTUP**: 27 jenis dokumen benar-benar ditegakkan · katalog penomoran tidak lagi buta terhadap seri yang tak terdaftar

> **Permintaan pemilik:** *"lanjutkan development dari repo ini"* (melanjutkan sesi yang berhenti
> saat memperbaiki tiga gate merah: INV-F14, INV-F3/MKTFULFILL, INV-F10).

## 0) MEMBUKA SESI — LINGKUNGAN DIPULIHKAN & TIGA GATE MERAH DIUKUR ULANG

`/app` datang sebagai template kosong; kode diambil dari `github.com/skaksudggd/DA`
(setelah pemilik menjadikannya publik) lalu `bash scripts/bootstrap.sh` (deps, seed, 6 login 200).
Tiga gate yang MERAH pada receipt terakhir (`2026-08-18 19:57`) dijalankan ulang **sebelum**
menulis kode apa pun:

| Gate | Hasil sesi ini |
|---|---|
| INV-F14 — form wajib memakai Master | **34/34 LULUS** (perbaikan sesi lalu memang sudah landing) |
| INV-MKTFULFILL (F3) — status pengiriman + pemulihan impor | **55 PASS · 0 GAGAL** |
| INV-F10 — layar daftar marketing | **21/21 HIJAU** |
| `bash scripts/gate.sh` (seluruh suite) | **VERDICT HIJAU** |

⇒ Pekerjaan sesi lalu SELESAI. Yang tersisa dari roadmap: **Fase G** — "41 jenis dokumen masih
'Otomatis saja'" (setelah sesi #19 tinggal **17 jenis** `pending_enforce`).

## 1) CACAT YANG DITEMUKAN SAAT BOOTSTRAP (bukan dugaan — bootstrap mencetaknya)

`seed_finance_wms_demo` melaporkan **4 gagal**: seeder mengirim `roll_no: DEMO-RL-000x`, padahal
Fase G menegakkan Roll Kain sebagai OTOMATIS ⇒ HTTP 400 *"nomor tidak boleh diketik"*. Akibatnya
layar **Roll Kain kosong** di setiap klon baru dan agen berikutnya menyangka fiturnya rusak.
Diperbaiki: seeder tidak lagi mengirim nomor, dan idempotensinya memakai **tanda tangan
(bahan · warna · lot) + penanda DEMO** — bukan nomor. Hasil: `0 gagal`, dijalankan dua kali
`0 dibuat · 10 dilewati`.

## 2) FASE G BATCH-3 — 17 JENIS TERSISA DIKERJAKAN, PLUS 10 JENIS YANG "DITEGAKKAN" TAPI MUSTAHIL DIPAKAI

### a. 15 jenis dokumen disambungkan ke satu pintu `issue_number`
| Kelompok | Jenis |
|---|---|
| SDM & Keuangan (3A) | Klaim Biaya · Permohonan Perjalanan Dinas · Penyelesaian Dinas · Transfer Bank · Order Penjualan |
| Produksi/Aksesoris/Marketing (3B) | Permak · Permintaan Komponen Kurang · Permintaan Aksesoris · Permintaan Kreator · **Retur Produksi (RTN)** |
| Batch-3C | Pengeluaran Barang Jadi · Permintaan Beli Aksesoris · Sampel Maklon · **Aset Tetap** · Aset Inventaris |

**Tiga di antaranya menerima nomor ketikan TANPA pemeriksaan apa pun** sebelum sesi ini
(`body.get('request_code') or gen_prefixed_number(...)` — Permintaan Komponen, Permintaan
Aksesoris, Permintaan Kreator): pola bebas diterima, nomor kembar tidak dicek.

### b. 10 jenis "ditegakkan" yang formnya TIDAK punya kolom nomor
Mode MANUAL untuk jenis-jenis ini berarti **dokumennya mustahil dibuat dari layar** (backend
meminta nomor yang tak punya kolom). Kolom nomor (`<DocNumberField>`) dipasang di: Pengeluaran FG ·
PR Aksesoris · Sampel Maklon · Aset Tetap · Aset Inventaris · **Penerimaan FG dari CMT** ·
**Invoice Maklon** · **Invoice Piutang (AR)** · Pengajuan Pinjaman (form kasbon, kuncinya belum
terdaftar di gate) · SPP/PO Maklon Produksi (sudah punya kolom sendiri sejak #18 — kini terdaftar).

### c. Tiga jenis DICABUT/DIKLASIFIKASI ULANG — setelan yang tidak mungkin dipakai lebih buruk daripada tidak ada
| Jenis | Keputusan | Alasan yang diukur |
|---|---|---|
| **Pinjaman Karyawan (legacy)** | dihapus dari katalog + `POST .../employee-loans/disburse` → **HTTP 410** | koleksi `rahaza_employee_loans` diarsipkan migrasi T2.1 (**0 dokumen**), menu diarahkan ke `hr-kasbon`, layarnya dilepas dari registry. Yang tertinggal hanya endpoint penulis ⇒ dua pintu pinjaman & dua penomoran |
| **Retur Material Produksi** | dihapus dari katalog (pengecualian beralasan di gate) | menu `prod-material-returns` DI-DEPRECATE pemilik (duplikasi), **tidak ada satu pun layar** yang membuatnya, koleksi kosong |
| **Roll Kain**, **PO Maklon (koleksi maklon)** & **Order Penjualan** | `policy_enforced` → `auto_only` + alasan | Roll lahir dari rincian penerimaan kain (belasan sekaligus) — bila owner memilih MANUAL, **penerimaan kain justru GAGAL**. PO Maklon dibuat di Portal Produksi lalu dicerminkan; layar lamanya sudah diarsipkan |

### d. DUA CACAT DATA yang ikut ketemu (dan diperbaiki)
1. **Kode Aset Tetap**: jalur kapitalisasi otomatis dari penerimaan barang memanggil generator
   dengan nama field **`asset_code`** padahal field dokumennya **`code`** ⇒ penyemaian counter
   membaca field yang tidak pernah ada ⇒ nomor bisa mulai dari 0 lagi. Selain itu form
   mewajibkan staf mengetik kode **tanpa satu pun aturan bentuk**.
2. **Format di katalog tidak sama dengan kenyataan** untuk 4 jenis: Sampel Maklon
   (`SMP-{YYYY}{MM}` vs nyatanya `SMP-<ORDER>-`), Aset Inventaris (`AST-{YYYY}` vs
   `AST-<KATEGORI>-<TAHUN>-`), Permintaan Komponen & Permintaan Aksesoris (prefix per jenis
   permintaan). Katalog kini memakai token (`{ORDER}`, `{KATEGORI}`, `{TIPE}`) sehingga
   **pratinjau di layar = nomor yang benar-benar akan lahir**.

### e. Satu lubang penjaga lagi ditutup: **form yang tidak bisa dibuka**
`RahazaOrdersModule.jsx` (Order Penjualan) MASIH ada sebagai berkas dan membaca kebijakan, tetapi
menunya sudah dinonaktifkan (`prod-orders` → redirect ke PO Internal) ⇒ "formnya" tak bisa dibuka
siapa pun. G7 kini memeriksa **keterjangkauan**: berkas form harus dirutekan di `moduleRegistry.js`
(termasuk lewat alias `lazy(() => import(...))` dan tab hub) atau dipakai komponen lain. Karena
itu Order Penjualan diklasifikasi ulang menjadi `auto_only` beserta alasannya.

## 3) PENJAGA BARU — G10..G13 (INV-F25: 9 → **13 invarian**)

| Invarian | Yang dijaga |
|---|---|
| **G10** | 5 jenis batch-3A ditegakkan pada DOKUMEN SUNGGUHAN (otomatis menolak ketikan · manual menolak kosong/pola bebas/kembar). Transfer Bank hanya jalur penolakan — menulisnya memposting jurnal GL, dan itu **dinyatakan di hasil gate**, bukan disembunyikan |
| **G11** | pintu pinjaman legacy benar-benar mati (410 + menyebut jalur yang benar) & tidak lagi ditawarkan di katalog |
| **G12** | **katalog WAJIB memuat setiap seri nomor yang benar-benar dibuat kode.** G9 hanya memeriksa entri yang SUDAH ada — ia buta terhadap yang tak pernah didaftarkan. Audit menemukan **5 seri hidup di luar katalog**: Retur Produksi (RTN) · Job Produksi · Job Cetak Barcode · Kode Supplier · seri WO yang bahkan sudah mati. Pengecualian harus TERTULIS beserta alasannya |
| **G13** | 5 jenis batch-3B ditegakkan pada dokumen sungguhan (termasuk Permak, yang bila PO uji tak ditemukan dilaporkan **TIDAK TERUKUR = MERAH**, bukan "lulus dengan sopan") |
| **G4 (diperbaiki)** | kunci "belum ditegakkan" tidak lagi di-hardcode — dipilih dari registry. Dua sesi berturut-turut gate ini merah karena kunci ujinya IKUT ditegakkan pada sesi itu |
| **G7 (diperketat)** | setiap kunci `policy_enforced` WAJIB terdaftar punya form yang membaca kebijakan (komponen bersama **atau** `/doc-number-policy`) — aturan inilah yang menemukan 10 jenis di §2b |

**Fungsi mati dibereskan:** `_gen_wo_number` (dewi_maklon.py) tidak pernah dipanggil sejak engine
`rahaza_work_orders` diarsipkan; isinya diganti galat yang menjelaskan, supaya audit tidak melihat
seri nomor hantu.
...
| Alat | Hasil |
|---|---|
| `python3 scripts/verify_fase_g2_penomoran_ditegakkan.py` | **HIJAU 13 invarian** (G1–G13) |
| Katalog | **51 jenis**: 27 ditegakkan · 23 selalu otomatis (beralasan) · **1 menunggu** (Kode Supplier — perlu keputusan pemilik: pola supplier sering mengikuti identitas luar) |
| Probe runtime batch-3C | ACC-PR-0003/ACC-PR-9901 · FA-00001/FA-99001 · AST-AP-2026-0001 · SMP-PO-MK-DEMO-2-01 — semua jalur penolakan 400 sesuai |
| `bash scripts/gate.sh` | lihat `memory/GATE_RECEIPT.md` sesi ini |

## 5) SISA (untuk sesi berikutnya)
1. **Kode Supplier** (`rahaza_suppliers.code`) — satu-satunya `pending_enforce`. Pertanyaan untuk
   pemilik: apakah kode supplier harus berpola (`SUP-0001`) atau tetap bebas mengikuti identitas
   supplier? Menegakkan pola tanpa keputusan itu = memaksa aturan pada data milik pihak luar.
2. **`da_assets.asset_code`** (modul Aset SDM) memakai `AST-{uuid6}` — **acak, tidak urut, tidak
   ada di katalog**, dan tidak tertangkap G12 (G12 memindai pemanggil `gen_prefixed_number`).
   Dua sistem aset hidup berdampingan (`da_assets` vs `dewi_assets`) — perlu keputusan konsolidasi.
3. Runtime invarian untuk batch-3C (5 jenis) masih dijaga STATIK (G1/G7/G12) + probe manual;
   menjadikannya G14 butuh penyiapan stok FG & jadwal depresiasi di dalam gate.
